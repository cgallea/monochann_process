...
def extract_coords(file):
    with segyio.open(file, mode='r+', ignore_geometry=True) as f:
        ...
        trace_headers = DataFrame(index=range(1, n_traces + 1), columns=headers.keys())
...
def open_segy_file():
    ...
    def save_limits():
        ...
        scenes = DataFrame({'img':[],'cmin':[],'cmax':[],'tit':[],'sigfilt':[],'cmap':[]}, dtype=object)
        tit = 'Raw Signal'
        sigfilt = ''
        cmap = palet
...
def ampl_filter():
    ...
    def hilbert():
        ...
        def show_if():
            ...
            cminIF, cmaxIF, cmapIF = img_show(segyfile, tit, IF, tri, trf, spi, spf, sr, cmin, cmax, palet, sigfil, trace_indext)
...
def conv_deconv():
    ...
    def deconv_boom():    
        ...
        def pred_decon():
            ...
            cminpDEC, cmaxpDEC, cmappDEC = img_show(filename, tit, prDEC, tri, trf, spi, spf, sr, cmin, cmax, palet, sigfilt, trace_index)
        ...
        def decon_with_pulse():
            ...
            cminbDEC, cmaxbDEC, cmapbDEC = img_show(filename, tit, pDEC, tri, trf, spi, spf, sr, cmin, cmax, palet, sigfilt, trace_index)
...
def interpret():
    ...
    def save_horiz():   # save digitalized horizonts
        ...
        if no_coords == True:
            ...
            refs = DataFrame(D, columns= ['Ref', 'Trace', 'Sample'])
            refs_coords = refs.merge(refs, coords) 
            ...
            df = DataFrame(H, columns= ['Ref','Trace','Xcoord','Ycoord','Depth','Intens'])
            ...
            try:
                with segyio.open(filename, ignore_geometry=True) as f:
                    ...
                    if chkCSV.get() == 1:
                        ...
                        df = DataFrame(D, columns= ['TRACE_INDEX', 'EAST', 'NORTH'])
