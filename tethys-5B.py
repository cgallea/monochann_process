...
class SeismicApp:
    def __init__(self, root):
        ...
        self.options = ['Raw Data', 'Spherical Divergence', 'Swell Filter', 'Band Pass', 'Ormsby','Spectral Whitening',
                        'AGC', 'STALTA', 'Conv Ricker', 'Conv Ormsby','Deconv Bottom', 'Deconv Predictive','Deconv Wavelet']
    ...
    def _update_sigdata(self, selection):
        ...
        try:
            if selection == 'Raw Data':
                ...
                self.cmap = self.palet  
            # elif selection == 'DC removal':
            #     self.sigdata = self.dc_data
            #     self.cmin = self.dc_data.min()
            #     self.cmax = self.dc_data.max()
            #     self.sigfilt = ' over DC removal'
            #     self.cmap = self.palet  
        ...
        try:
            if selection == 'Raw Data':
                ...
                if not sw_scene.empty:
                    ...
                    self.cmap = self.palet
            # elif selection == 'DC removal':
            #     dc_scene = self.scenes[self.scenes['tit'].str.contains('DC removal')]
            #     if not dc_scene.empty:
            #         self.sigdata = dc_scene.iloc[-1]['img']
            #         self.cmin = dc_scene.iloc[-1]['cmin']
            #         self.cmax = dc_scene.iloc[-1]['cmax']
            #         self.sigfilt = ' over DC removal'
            #         self.cmap = self.palet
            elif selection == 'Band Pass':
                bp_scene = self.scenes[self.scenes['tit'].str.contains('Band Pass')]
    ...
    def _save_filter_result(self, filter_type):
        ...
        # elif filter_type == 'DC removal' and self.dc_data is not None:
        #     scene = [self.dc_data, self.cmin, self.cmax, self.dc_title, self.sigfilt, self.cmap]
        #     self.scenes.loc[len(self.scenes)] = scene
        #     self.dc_window.destroy()
    ...
    def open_corrections_window(self):
        ...
        # Button(corrections_window, text='DC remove', width=22, command=self.apply_dc_remove_window,
        #        bg="#A39C6A", fg='white').place(x=50, y=160)
    ...
    # def apply_dc_remove_window(self):

    #     sig1d = np.ravel(self.sigdata)
    #     m = np.mean(sig1d)
    #     dc = list(map(lambda var: m - var, sig1d)) 
    #     sp = int(len(sig1d) / self.nt)
    #     dc = np.reshape(dc, (self.nt, sp))

    #     tr = self.nt//2
    #     plt.figure(figsize=(12, 4))
    #     plt.title('Check if signal is not centered around the zero level axis')
    #     y = self.sigdata[tr]   # plot a trace
    #     plt.plot(y, label=' Raw Trace '+str(tr))
    #     plt.hlines(0, 0, self.ns, color='k', linestyles='dashdot')
    #     plt.plot(dc[tr], label=' Raw trace with DC Component correction')
    #     plt.legend()
    #     plt.xlabel('Samples')
    #     plt.ylabel('Signal')
    #     # plt.title(' Line ' + self.filename + ' - Trace DC Component')
    #     plt.grid()

    #     show_btn = plt.axes([0.75, 0.95, 0.15, 0.05])   # define button area
    #     # put the matplotlib button in that area
    #     btn = Btn(show_btn, 'Apply DC Correction', hovercolor='gold')
    #     btn.on_clicked(lambda event: self.apply_dc_correction())
    #     plt.show()

    # def apply_dc_correction(self):
                    
    #     # remove DC component from seismic data
    #     sig1d = np.ravel(self.sigdata)  # convert 2D array to 1D array
    #     m = np.mean(sig1d)
    #     dc = list(map(lambda var: m - var, sig1d))
    #     sp = int(len(sig1d)/self.nt)
    #     self.dc_data = np.reshape(dc, (self.nt, sp))

    #     vm = np.percentile(self.dc_data, 95)
    #     self.cmin, self.cmax = -vm, vm
    #     self.sd_title = 'DC remove Correction'
    #     self.sigfilt = ' over DC remove correction' 
    #     self.cmap = self.palet
    ...
    #     vm = np.percentile(self.dc_data, 95)
    #     axes[1].imshow(self.dc_data.T, cmap=self.palet, aspect='auto', extent=[0, self.nt, self.ns*self.sr, 0], vmin=-vm, vmax=vm)
    #     axes[1].set_title('DC Removal', fontsize=12)
    #     axes[0].set_xlabel('Trace Number')
    #     axes[1].set_ylabel('Time (s)')
    ...
    #     def save_sd_correction(self):
    #         self.sd_title = 'DC removal Correction'
    #         self.cmap = self.palet
    #         self._save_filter_result('DC removal')


    #     # self.save_correction(self.dc_data)

    #     # show_btn = plt.axes([0.75, 0.95, 0.15, 0.05])   # define button area
    #     # # put the matplotlib button in that area
    #     # btn = Btn(show_btn, 'Save DC correction', hovercolor='gold')
    #     # btn.on_clicked(lambda event: self.save_correction(self.dc_remove_data))
