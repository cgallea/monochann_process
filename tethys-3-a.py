...
def img_show(filename, txt, im, tri, trf, spi, spf, sr, v_min, v_max, colrs, filt):
    ...
    vm = np.percentile(im, 99) # im is the filtered image
    img = ax1.imshow(im.T, vmin=-vm, vmax=vm, cmap=colrs, aspect='auto', interpolation='nearest', extent=ext)

    plt.subplots_adjust(top=0.94, bottom=0.08, left=0.04, right=0.925) # define chart area
    ...
    showbtn = plt.axes([0.83, 0.95, 0.1, 0.05])   # define button area
...
def conv_deconv():
    ...
    def deconv_boom():    
        ...
        def view_section():
            ...
            ax.set_title('Select the window for capturing the bottom signal')
        ...
        def extract_pulse():
            ...
            plt.title('Bottom reflection area window display')
    ...
    def intpret():
        ...
        class PolygonDrawer:
            ...
            def __init__(self, digit):
                ...
                self.clear_button = Button(self.button_frame, text="    Clean    ", command=self.clear_polygons)
