...
def select(window, px, py):
        ...
        def check_cbox(event):
            ...
            try:
                ...
                if cbox.get() == 'Ricker Convol':
                    filt = ' over Ricker Convolution'
                    assig(RIK, cminRIK, cmaxRIK, palet, filt)

                if cbox.get() == 'Deconv-Bottom':
                    filt = ' over Bottom Capture Deconvolution'
                    assig(bDEC, cminbDEC, cmaxbDEC, palet, filt)

                if cbox.get() == 'Deconv-Pulse':
                    filt = ' over Signal Pulse Deconvolution'
                    assig(pDEC, cminpDEC, cmaxpDEC, palet, filt)

                if cbox.get() == 'Deconv-Predict':
                    filt = ' over Predidtive Deconvolution'
                    assig(prDEC, cminprDEC, cmaxprDEC, palet, filt)
        ...
        options = ['Raw Data','Band Pass', 'Spectr Whiten','Parm Equaliz','AGC','Manual Gain','STA/LTA','Ricker Convol',
            'Bottom Deconv','Pulse Deconv','Predict Deconv','Inst Amplit','Inst Freq','Phase Cos','Laplace','x-Deriv','y-Deriv','Wiener']
...
# ===================================================================================================
    ...
    def spher_diver():
        ...
        def save_sd():
            ...
            btn2['state'] = 'disabled'
            txt_edit.insert(END, '\n')
            txt_edit.insert(END, '\nSpherical Divergence Correction applied over Raw Data')
...
# ===================================================================================================
    ...
    def resize_segy():
        ...
        lbl4 = Label(resize, text='Does not Save Coordinates')
        lbl4.place(x=70, y=180)
...
def conv_deconv():
    ...
    def deconv_bot():    
        ...
        deconv_bot = Toplevel(conv_deconv)
        deconv_bot.title('DECONVOLUTION')
        # deconvolution.iconbitmap("logotet.ico")
        w = 320 # window width
        h = 480 # window height
        ws = deconv_bot.winfo_screenwidth()
        hs = deconv_bot.winfo_screenheight()
        x = (ws/2) - (w/2)
        y = (hs/2) - (h/2)
        deconv_bot.geometry('%dx%d+%d+%d' % (w, h, x, y))
        deconv_bot.resizable(width=False, height=False)
        deconv_bot.attributes('-toolwindow', True)   

        lbl1 = Label(deconv_bot, text='Select Seabottom Window Pulse')
        lbl1.place(x=75, y=65)

        btn1 = Button(deconv_bot, width=15, text='Show Image', command= view_section)
        btn1.place(x=100, y=90)

        btn2 = Button(deconv_bot, width=15, text='Show Pulse', command= extract_pulse)
        btn2.place(x=100, y= 130)

        btn3 = Button(deconv_bot, width=20, text='Deconvol with Bottom', command= decon_bottom)
        btn3.place(x=80, y= 170)

        btn6 = Button(deconv_bot, width=20, text='Save', command= save_decon_bottom)
        btn6.place(x=80, y= 210)  

        btn4 = Button(deconv_bot, width=20, text='Deconvol with Pulse', command= decon_with_pulse)
        btn4.place(x=80, y= 250)

        btn6 = Button(deconv_bot, width=20, text='Save', command= save_decon_pulse)
        btn6.place(x=80, y= 290)  

        btn5 = Button(deconv_bot, width=20, text='Preditive Deconvol', command= pred_decon)
        btn5.place(x=80, y= 330)

        btn6 = Button(deconv_bot, width=20, text='Save', command= save_pred_decon)
        btn6.place(x=80, y= 370)        

        select(deconv_bot, 110, 30)
    ...
    btn1.place(x=40, y=40)
    btn2 = Button(conv_deconv, width=15, text='DECONVOLUTION', command= deconv_bot)
...
def imag_filters():
    ...
    def y_deriv():
        ...
        DY = np.diff(sigdata, axis = 1)  # compute y-derivativeFt
...
========
=======
>>>>>>> ad5c44c (Atualização do repositorio)
import numpy as np
import math
import os, sys
import matplotlib as mpl
...
<<<<<<< HEAD
                ...
                if cbox.get() == 'Boom Deconv':
                    filt = ' over Boomer deconvolution'
                    assig(bDEC, cminbDEC, cmaxbDEC, palet, filt)
...
<<<<<<< HEAD
def pal_choice(event):  # palettes divergentes
=======
def pal_choice(event):  # divergent pallets
>>>>>>> ad5c44c (Atualização do repositorio)
    global palet
...
class ManualGain(object):
    ...
    def on_key(self, event):
        ...
        if event.key == 'escape':
            ...
            if self.escap == False:
<<<<<<< HEAD
                # interpola o traço entre os vertices da linha desenhada 
=======
                # interpolates the stroke between the vertices of the drawn line 
>>>>>>> ad5c44c (Atualização do repositorio)
                if self.trc[0] < self.xs[0]: 
                ...
                self.fig.canvas.draw_idle()
<<<<<<< HEAD
                self.escap = True  # ja foi pressionado o escape

        if event.key == 'x':
            plt.cla(self.ax1) # apaga linha desenhada
            plt.cla(self.ax2) # apaga traco interpolado
=======
                self.escap = True  
...
<<<<<<< HEAD
def img_show(filename, txt, im, tri, trf, spi, spf, sr, v_min, v_max, colrs, filt):
=======
def img_show(filename, txt, im, tri, trf, spi, spf, sr, v_min, v_max, colrs, filt, trace_index):
>>>>>>> ad5c44c (Atualização do repositorio)
    global ext
...
<<<<<<< HEAD
    ...
    ax1_smp.set_xlim(tri, trf)
    ax1_smp.figure.canvas.draw()
    ax1.set_title(f'{filename}'+'  -  '+txt+filt, fontsize=12)

    vm = np.percentile(im, 99) # im é a imagem filtrada
    img = ax1.imshow(im.T, vmin=-vm, vmax=vm, cmap=colrs, aspect='auto', interpolation='nearest', extent=ext)

    plt.subplots_adjust(top=0.94, bottom=0.08, left=0.04, right=0.925) # define area do chart
    #axcolor = 'lightgoldenrodyellow'    
    ax_clr_min = plt.axes([0.96, 0.1, 0.01, 0.8]) # posicao dos sliders na area criada
    ax_clr_max = plt.axes([0.98, 0.1, 0.01, 0.8])
    s_clr_min = Slider(ax_clr_min, '', v_min, 0, orientation='vertical') # coloca os slides 
    s_clr_max = Slider(ax_clr_max, '', 0, v_max, orientation='vertical')
 
    showbtn = plt.axes([0.83, 0.95, 0.1, 0.05])   # define a area do botão
    # coloca o batão do matplotlib nessa area
=======
    # ----------------------------------------------------------------------------
...
<<<<<<< HEAD
        _clr_min = s_clr_min.val # pega o valor cmin do slider
        _clr_max = s_clr_max.val # pega o valor cmax do slider
        img.set_clim(_clr_max) # seta cmin, cmax na imagem
        img.set_clim([_clr_min, _clr_max]) 
        cmin, cmax = store(_clr_min, _clr_max) # guarda _clr_min em cmin e _clr_max em cmax
        plt.draw()  # redesenha a imagem com novos cmin e cmax
=======
        _clr_min = s_clr_min.val # get the cmin value from the slider
        _clr_max = s_clr_max.val # get the cmax value from the slider
        img.set_clim(_clr_max) # cmin, cmax arrow in the image
...
<<<<<<< HEAD
    global data, nt, ns, sr, twt, coords, head, datatype, spec, f
    global filepath, segyfile, filename, no_coords, sigdata, logfile
=======
    global data, nt, ns, sr, twt, coords, head, datatype, spec, f, trace_index
    global filepath, segyfile, filename, no_coords, sigdata  #, logfile
...
>>>>>>> ad5c44c (Atualização do repositorio)
    ...
    if min(data[10]) < 0:
<<<<<<< HEAD
        datatype = 0   # dados analiticos
        txt_edit.insert(END, '\n')
        txt_edit.insert(END, '\n Analitic Data')
    else:
        datatype = 1     # dados envelopados
=======
        datatype = 0   # analitic data
        txt_edit.insert(END, '\n')
        txt_edit.insert(END, '\n Analitic Data')
...
<<<<<<< HEAD
        ...
        sig = data[tri:trf, spi:spf]   # define arquivo de trabalho na janela selecionada
        sigdata = sig                  # sigdata é usado no select data e define a imagem escolhida
        sig1d = np.ravel(sig)

        vm = np.percentile(sig, 99)   # obtem o percentil dos dados brutos pra iniciar o viewer
=======
        spi = int(tmi /sr /1000 )  # initial sample
        spf = int(tmf /sr /1000 )  # final smaple
        tr = int((trf-tri)/2)
...
>>>>>>> ad5c44c (Atualização do repositorio)
        ...
        sigfilt = ''
<<<<<<< HEAD
        cmin, cmax, cmap = img_show(segyfile, tit, sig, tri, trf, spi, spf, sr, cmin, cmax, palet, sigfilt)
=======
        cmin, cmax, cmap = img_show(segyfile, tit, sig, tri, trf, spi, spf, sr, cmin, cmax, palet, sigfilt, trace_index)
>>>>>>> ad5c44c (Atualização do repositorio)
...
<<<<<<< HEAD
        scenes = pd.DataFrame({'img':[],'cmin':[],'cmax':[],'tit':[],'sigfilt':[],'cmap':[]})
=======
        scenes = pd.DataFrame({'img':[],'cmin':[],'cmax':[],'tit':[],'sigfilt':[],'cmap':[]}, dtype=object)
>>>>>>> ad5c44c (Atualização do repositorio)
        tit = 'Raw Signal'
...
<<<<<<< HEAD
    btn4 = Button(work_window, width=2, height=1, text ='?', command= lambda:pdfReader(r'C:\tethys\help\work_window.pdf'), font=('Arial', 9, 'bold'))
...
<<<<<<< HEAD
            cmin, cmax, cmap = img_show(segyfile, tit, sigdata, 0, nt, 0, ns, sr, cmin, cmax, palet, sigfilt)
=======
            cmin, cmax, cmap = img_show(segyfile, tit, sigdata, 0, nt, 0, ns, sr, cmin, cmax, palet, sigfilt, trace_index)
>>>>>>> ad5c44c (Atualização do repositorio)
...
<<<<<<< HEAD
            ...
            # def movaver(x, w):
            #         return pd.DataFrame(x).rolling(w,center=True).mean()
=======
>>>>>>> ad5c44c (Atualização do repositorio)
...
<<<<<<< HEAD
            # extrai Smax (NUMERO DA AMOSTRA) do traço 
=======
            # extracts Smax (SAMPLE NUMBER) from the trace
>>>>>>> ad5c44c (Atualização do repositorio)
            def extractSmax(trace, sta, lta):
...
<<<<<<< HEAD
            ...
            for tr in range(nt):  
                sigtrace = sigdata[tr, smp_ini:]
                smax = extractSmax(sigtrace, n_sta, n_lta)  # usando os que melhor funcionaram (colocar input pra escolher)
                Smax = np.append(Smax, smax)
            # numero da amostra da intensidade max de cada um dos nt traços

            fator = 1.5                                #1.5 é o fator de multiplicacao
            q75, q25 = np.percentile(Smax, [75, 25])   #retorna o terceiro e primeiro quartil
            ...
            for i in range(nt):                        # interpola o intervalo do outlier
                if Smax[i] > highpass:
                    Smax[i] = Smax[i-1] # usa o anterior

                if Smax[i] < lowpass:
                    Smax[i] = Smax[i-1] # usa o anterior 

            # aplicando a media movel numa janela de win amostras
            win = 100
            # sma = moving_average(Smax, win)
            sma = moving_average(Smax, win)

            # valor traco inicial = valor traco inicial+window//2
            # valor traco final = valor traco final-window//2
=======
...
<<<<<<< HEAD
            # Criando a spline cúbica sobre o moving average
=======
            # Creating the cubic spline over the moving average
...
<<<<<<< HEAD
            # diferenca entre a spline e o theshold em cada ponto
            # corresponde ao numero de amostras entre as duas
            dif = (cs_y - Smax) 
            dif = dif.astype(int)

            ################ METODO 1 usando insert e del)
            
            sigcor = sigdata.tolist()  # converte array para lista
...
>>>>>>> ad5c44c (Atualização do repositorio)
            for tr in range(nt):
                ...
                if dif[tr] > 0:
                    ...
                    for i in range(int(dif[tr])):
<<<<<<< HEAD
                        # inserir zero no topo do traco

                        sigcor[tr].insert(i, 0)         
                                    
                        # remover amostras da base do traco
=======
...
>>>>>>> ad5c44c (Atualização do repositorio)
                ...
                if dif[tr] >= 0 :
                    for i in range(int(dif[tr])):
<<<<<<< HEAD
                        # remover amostras do topo do traco
                        del sigcor[tr][-i]
                        
                        # inserir zeros na base do traco
                        sigcor[tr].insert(i, 0)
                        
            sigcor = np.array(sigcor)  # volta para array

            tit = ' SWELL Filter - METODO 1'
            cmin = -np.percentile(sigcor, 95)
            cmax = np.percentile(sigcor, 95)
            sigfilt = '' 
            cmin, cmax, cmap = img_show(segyfile, tit, sigcor, 0, nt, 0, ns, sr, cmin, cmax, palet, sigfilt)

            #####################  METODO 2  usando a função shift
=======
...
<<<<<<< HEAD
            tit = ' SWEll Filter - METODO 2'
            cmin = -np.percentile(correct, 95)
            cmax = np.percentile(correct, 95)
            sigfilt = ''
            cmin, cmax, cmap = img_show(segyfile, tit, correct, 0, nt, 0, ns, sr, cmin, cmax, palet, sigfilt)
=======
            tit = ' SWEll Filter - METHOD #2'
            cmin = -np.percentile(correct, 95)
            cmax = np.percentile(correct, 95)
...
<<<<<<< HEAD
            tit = ' SWELL Filter - METODO 1'
            sigfilt = ''
            # cmin, cmax, cmap = img_show(segyfile, tit, sig, 0, nt, 0, ns, sr, cmin, cmax, palet, sigfilt)
=======
            tit = ' SWELL Filter - METHOD #1'
            sigfilt = ''
            # cmin, cmax, cmap = img_show(segyfile, tit, sig, 0, nt, 0, ns, sr, cmin, cmax, palet, sigfilt, trace_index)
...
<<<<<<< HEAD
        btn5 = Button(swell, width=2, height=1, text ='?', command= lambda:pdfReader(r'C:\tethys\help\swell_filter.pdf'), font=('Arial', 9, 'bold'))
        btn5.place(x=10, y=360)
...
<<<<<<< HEAD
            cmin, cmax, cmap = img_show(segyfile, tit, dcremove, 0, nt, 0, ns, sr, cmin, cmax, palet, sigfilt)
=======
            cmin, cmax, cmap = img_show(segyfile, tit, dcremove, 0, nt, 0, ns, sr, cmin, cmax, palet, sigfilt, trace_index)
>>>>>>> ad5c44c (Atualização do repositorio)
...
<<<<<<< HEAD
        btn4 = Button(DCremove, width=2, height=1, text ='?', command= lambda:pdfReader(r'C:\tethys\help\dc_remove.pdf'), font=('Arial', 9, 'bold'))
...
<<<<<<< HEAD
            def spher_diverg(dat, ntraces, nsamples):
...
<<<<<<< HEAD
            SD = spher_diverg(sigdata, sigdata.shape[0], sigdata.shape[1])

            vm = np.percentile(SD, 95)   # obtem o percentil dos dados brutos 
=======
            vel = 1500   # m/s 
            SD = spher_diverg(sigdata, sigdata.shape[0], sigdata.shape[1], vel)
...
<<<<<<< HEAD
            cmin, cmax, cmap = img_show(segyfile, tit, SD, 0, nt, 0, ns, sr, cmin, cmax, 'gray', sigfilt) 
=======
            cmin, cmax, cmap = img_show(segyfile, tit, SD, 0, nt, 0, ns, sr, cmin, cmax, 'gray', sigfilt, trace_index) 
>>>>>>> ad5c44c (Atualização do repositorio)
...
<<<<<<< HEAD
        btn4 = Button(spher_div, width=2, height=1, text ='?', command= lambda:pdfReader(r'C:\tethys\help\spheric_diverg'), font=('Arial', 9, 'bold'))
        btn4.place(x=5, y=170)
...
<<<<<<< HEAD
            ...
            seg = data[tri:trf, spi:spf]   # define arquivo de trabalho na janela selecionada

            vm = np.percentile(seg, 99)   # obtem o percentil dos dados brutos pra iniciar o viewer
=======
            spi = int(tmi /sr /1000 )  # initial sample
            spf = int(tmf /sr /1000 )  # final sample
...
<<<<<<< HEAD
            cmin, cmax, cmap = img_show(segyfile, tit, seg, tri, trf, spi, spf, sr, cmin, cmax, palet, sigfilt)
=======
            cmin, cmax, cmap = img_show(segyfile, tit, seg, tri, trf, spi, spf, sr, cmin, cmax, palet, sigfilt, trace_index)
>>>>>>> ad5c44c (Atualização do repositorio)
...
<<<<<<< HEAD
        btn4 = Button(resize, width=2, height=1, text ='?', command= lambda:pdfReader(r'C:\tethys\help\resize'), font=('Arial', 9, 'bold'))
        btn4.place(x=10, y=220)
=======
        lbl4 = Label(resize, text='Does not Save Coordinates')
        lbl4.place(x=70, y=180)
...
<<<<<<< HEAD
    ...
    btn1 = Button(corrections, width=12, text="Swell filter", command=swell_filter)
...
>>>>>>> ad5c44c (Atualização do repositorio)
    ...
    if  datatype == 1: 
        btn2['state'] = DISABLED    
<<<<<<< HEAD
    btn3 = Button(corrections, width=20, text="Spherical Divergence", command=spher_diver)
...
<<<<<<< HEAD
        btn4 = Button(spectrum, width=2, height=1, text ='?', command= lambda:pdfReader(r'C:\tethys\help\spectrum.pdf'), font=('Arial', 9, 'bold'))
...
# ===================================================================================================  
    ...
    def band_pass():
<<<<<<< HEAD
        # curioso - aplicando BandPass o sinal envelopado passa a ser analitico
=======
...
<<<<<<< HEAD
            try:
                BP = butter_bandpass_filter(sigdata, LC, HC, 1/sr, order=ord) # order => angulo da rampa
                ...
                cminBP, cmaxBP, cmapBP = img_show(segyfile, tit, BP, tri, trf, spi, spf, sr, cmin, cmax, palet, sigfilt)
...
<<<<<<< HEAD
        ...
        btn4 = Button(band_pass, width=2, height=1, text ='?', command= lambda:pdfReader(r'C:\tethys\help\band_pass.pdf'), font=('Arial', 9, 'bold'))
        btn4.place(x=10, y=260)
=======
        # btn3 = Button(band_pass, width=12, text ='Save SEG-Y', command=save_bp_sgy)
        # btn3.place(x=80, y=220)
...
<<<<<<< HEAD
        fonte: wjchen
        em: https://github.com/chenwj1989/pafx
=======
        font: wjchen in
        https://github.com/chenwj1989/pafx
>>>>>>> ad5c44c (Atualização do repositorio)
...
<<<<<<< HEAD
            F9 = varf9.get()
            G9 = varg9.get()
            center_freqs = [F1, F2, F3, F4, F5, F6, F7, F8, F9]
=======
            center_freqs = [F1, F2, F3, F4, F5, F6, F7, F8]
>>>>>>> ad5c44c (Atualização do repositorio)
...
<<<<<<< HEAD
            BW9 = F9/np.sqrt(2)
            band_widths = [BW1, BW2, BW3, BW4, BW5, BW6, BW7, BW8, BW9]
=======
            band_widths = [BW1, BW2, BW3, BW4, BW5, BW6, BW7, BW8]
>>>>>>> ad5c44c (Atualização do repositorio)
...
<<<<<<< HEAD
            eq_gains = [G1, G2, G3, G4, G5, G6, G7, G8, G9]   # ganho de cada canal
=======
            eq_gains = [G1, G2, G3, G4, G5, G6, G7, G8]   # ganho de cada canal
>>>>>>> ad5c44c (Atualização do repositorio)
            fs = 1/sr
...
>>>>>>> ad5c44c (Atualização do repositorio)
            ...
            palet = sigpal
<<<<<<< HEAD
            tit = 'Parametric Equalizer - Gain: '+str(G1)+' '+str(G2)+' '+str(G3)+' '+str(G4)+' '+str(G5)+' '+str(G6)+' '+str(G7)+' '+str(G8)+' '+str(G9)

            cminEQ, cmaxEQ, cmapEQ = img_show(segyfile, tit, EQ, tri, trf, spi, spf, sr, cmin, cmax, palet, sigfilt)      
=======
            tit = 'Parametric Equalizer - Gain: '+str(G1)+' '+str(G2)+' '+str(G3)+' '+str(G4)+' '+str(G5)+' '+str(G6)+' '+str(G7)+' '+str(G8)
...
<<<<<<< HEAD
            logfile = open(filename+'.log', 'a')
            logfile.write('\nParametric Equalizer - Gain: ['+str(G1)+' '+str(G2)+' '+str(G3)+' '+str(G4)+' '+str(G5)+' '+str(G6)+' '+str(G7)+' '+str(G8)+' '+str(G9)+']'+sigfilt)
=======
            # logfile = open(filename+'.log', 'a')
            # logfile.write('\nParametric Equalizer - Gain: ['+str(G1)+' '+str(G2)+' '+str(G3)+' '+str(G4)+' '+str(G5)+' '+str(G6)+' '+str(G7)+' '+str(G8)+']'+sigfilt)
>>>>>>> ad5c44c (Atualização do repositorio)
...
<<<<<<< HEAD
        ...
        btn4 = Button(equaliz, width=2, height=1, text ='?', command= lambda:pdfReader(r'C:\tethys\help\param_equal'), font=('Arial', 9, 'bold'))
        btn4.place(x=10, y=340)
=======
        # btn3 = Button(equaliz, text='Save SEG-Y', width=12, command=save_equaliz_sgy)
        # btn3.place(x=70, y=290)
>>>>>>> ad5c44c (Atualização do repositorio)
...
<<<<<<< HEAD
            cminWT, cmaxWT, cmapWT = img_show(segyfile, tit, WT, tri, trf, spi, spf, sr, cmin, cmax, palet, sigfilt)
=======
            cminWT, cmaxWT, cmapWT = img_show(segyfile, tit, WT, tri, trf, spi, spf, sr, cmin, cmax, palet, sigfilt, trace_index)
>>>>>>> ad5c44c (Atualização do repositorio)
...
<<<<<<< HEAD
        ...
        btn4 = Button(whitening, width=2, height=1, text ='?', command= lambda:pdfReader(r'C:\tethys\help\spec_whiten.pdf'), font=('Arial', 9, 'bold'))
        btn4.place(x=10, y=270)
=======
        # btn3 = Button(whitening, width=10, text ='Save SEG-Y', command= save_whiten_sgy)
        # btn3.place(x=65, y=230)
>>>>>>> ad5c44c (Atualização do repositorio)
    ...
    btn1.place(x=40, y=40)
<<<<<<< HEAD
    btn1 = Button(freq_filters, text='BAND PASS', width=18, command=band_pass)
    btn1.place(x=40, y=90)
    btn1 = Button(freq_filters, text='SPECTRAL WHITEN', width=18, command=spectral_whitening)
    btn1.place(x=40, y=140)
    btn1 = Button(freq_filters, text='PARAM EQUALIZ', width=18, command=param_equal)
    btn1.place(x=40, y=190)
=======
    btn2 = Button(freq_filters, text='BAND PASS', width=18, command=band_pass)
    btn2.place(x=40, y=90)
    btn3 = Button(freq_filters, text='SPECTRAL WHITEN', width=18, command=spectral_whitening)
    ...
    def agc_gain():
        ...
        def apply_agc():                
            ...
            def agc(signal, window_size):
                """
<<<<<<< HEAD
                Aplica o Controle Automático de Ganho (AGC) a um sinal sísmico.

                Parâmetros:
                    - signal: array NumPy representando o sinal sísmico.
                    - window_size: tamanho da janela deslizante em amostras.

                Retorna:
                    - Sinal AGC ajustado.
                """
                # Calcula o quadrado do valor absoluto do sinal
                envelope = np.abs(signal) ** 2

                # Calcula a média móvel com a janela deslizante
                smoothed = np.convolve(envelope, np.ones(window_size) / window_size, mode='same')

                # Calcula a raiz quadrada da média móvel suavizada
                smoothed_sqrt = np.sqrt(smoothed)

                # Normaliza o sinal original pelo sinal suavizado
=======
                Applies Automatic Gain Control (AGC) to the seismic signal.
...
<<<<<<< HEAD
            cminAGC, cmaxAGC, cmapAGC = img_show(segyfile, tit, AGC, tri, trf, spi, spf, sr, cmin, cmax, palet, sigfilt)
...
<<<<<<< HEAD
        ...
        btn4 = Button(Agc, width=2, height=1, text ='?', command= lambda:pdfReader(r'C:\tethys\help\agc.pdf'), font=('Arial', 9, 'bold'))
    ...
    def manual_gain():
        ...
        def edit_mgain():
            ...
            if min(sigdata[50]) < 0:
                ...
                interv = nt
<<<<<<< HEAD
                xnew = np.linspace(draw_line.xs[0], draw_line.xs[-1], interv) # vetor com xy da linha
                # fint = scipy.interpolate.interp1d(draw_line.xs, draw_line.ys)
                # y = fint(xnew)  # interpolacao y desse vetor
                fint = make_interp_spline(draw_line.xs, draw_line.ys)
                y = fint(xnew)  # interpolacao y desse vetor
...
<<<<<<< HEAD
                cminMG, cmaxMG, cmapMG = img_show(segyfile, tit, MG, tri, trf, spi, spf, sr, cmin, cmax, palet, sigfilt)
=======
                cminMG, cmaxMG, cmapMG = img_show(segyfile, tit, MG, tri, trf, spi, spf, sr, cmin, cmax, palet, sigfilt, trace_index)
>>>>>>> ad5c44c (Atualização do repositorio)
...
<<<<<<< HEAD
        ...
        btn4 = Button(mgain, width=2, height=1, text ='?', command= lambda:pdfReader(r'C:\tethys\help\mgain.pdf'), font=('Arial', 9, 'bold'))
...
<<<<<<< HEAD
            cminSTA, cmaxSTA, cmapSTA = img_show(segyfile, tit, STA, tri, trf, spi, spf, sr, cmin, cmax, palet, sigfilt)
=======
            cminSTA, cmaxSTA, cmapSTA = img_show(segyfile, tit, STA, tri, trf, spi, spf, sr, cmin, cmax, palet, sigfilt, trace_index)
>>>>>>> ad5c44c (Atualização do repositorio)
...
<<<<<<< HEAD
        ...
        btn6 = Button(st_lt, width=2, height=1, text ='?', command= lambda:pdfReader(r'C:\tethys\help\stalta.pdf'), font=('Arial', 9, 'bold'))
...
<<<<<<< HEAD
            cminIA, cmaxIA, cmapIA = img_show(segyfile, tit, IA, tri, trf, spi, spf, sr, cmin, cmax, palet, sigfilt)
=======
            cminIA, cmaxIA, cmapIA = img_show(segyfile, tit, IA, tri, trf, spi, spf, sr, cmin, cmax, palet, sigfilt, trace_index)
>>>>>>> ad5c44c (Atualização do repositorio)
        ...
        def show_if():
            ...
            tit = 'Instant Frequency'
<<<<<<< HEAD
            cminIF, cmaxIF, cmapIF = img_show(segyfile, tit, IF, tri, trf, spi, spf, sr, cmin, cmax, palet, sigfilt)
=======
            cminIF, cmaxIF, cmapIF = img_show(segyfile, tit, IF, tri, trf, spi, spf, sr, cmin, cmax, palet, sigfilt, trace_index)
>>>>>>> ad5c44c (Atualização do repositorio)
...
<<<<<<< HEAD
            PC = np.cos(IP)  # extrai phase cosine
=======
            PC = np.cos(IP)  # extract phase cosine
>>>>>>> ad5c44c (Atualização do repositorio)
...
<<<<<<< HEAD
            cminPC, cmaxPC, cmapPC = img_show(segyfile, tit, PC, tri, trf, spi, spf, sr, cmin, cmax, palet, sigfilt)
=======
            cminPC, cmaxPC, cmapPC = img_show(segyfile, tit, PC, tri, trf, spi, spf, sr, cmin, cmax, palet, sigfilt, trace_index)
>>>>>>> ad5c44c (Atualização do repositorio)
        ...
        def show_accel():
            global AC, cminAC, cmaxAC, cmapAC, tit
<<<<<<< HEAD
            x = np.linspace(spi, spf, spf-spi) # array das amostras

            for tr in range(trf-tri): # calcula as derivadas de cada traco
                y = data[tr, spi:spf] 
                spl = UnivariateSpline(x, y) # cria spline sobre o traço
                drv_spl = spl.derivative() # derivada da spline
                
            AC = drv_spl(IF) # deriva a phase acceleration de todos os tracos
=======
...
<<<<<<< HEAD
            cminAC, cmaxAC, cmapAC = img_show(segyfile, tit, AC, tri, trf, spi, spf, sr, cmin, cmax, palet, sigfilt)
=======
            cminAC, cmaxAC, cmapAC = img_show(segyfile, tit, AC, tri, trf, spi, spf, sr, cmin, cmax, palet, sigfilt, trace_index)
>>>>>>> ad5c44c (Atualização do repositorio)
...
<<<<<<< HEAD
        ...
        btn11 = Button(transf, width=2, height=1, text ='?', command= lambda:pdfReader(r'C:\tethys\help\hilbert.pdf'), font=('Arial', 9, 'bold'))
...
def conv_deconv():
    ...
    def ricker(s):
        # waveket Ricker
        points = s  # num amostras por pulso
        a = 4.0
        return signal.ricker(points, a)
...
<<<<<<< HEAD
            # convolve usando a wavelet Ricker
=======
            # convolve using the Ricker wavelet
>>>>>>> ad5c44c (Atualização do repositorio)
...
<<<<<<< HEAD
            fs = 1 / sr  # frequencia de amostragem
            samples = fs*duration
            t = np.arange(int(samples)) / fs
...
>>>>>>> ad5c44c (Atualização do repositorio)
            ...
            palet = sigpal
<<<<<<< HEAD
            tit = ' Ricker Pulse Convoluted with Chirp Signal'

            cminRIK, cmaxRIK, cmapRIK = img_show(segyfile, tit, RIK, tri, trf, spi, spf, sr, cmin, cmax, palet, sigfilt)
=======
            tit = f' Convolution of Signal with Ricker (pulse width={pw} s)'
...
>>>>>>> ad5c44c (Atualização do repositorio)
        ...
        def save_rick_conv():
            ...
            txt_edit.insert(END, '\n')
<<<<<<< HEAD
            txt_edit.insert(END, '\nChirp Ricker convolution saved - Wavelet Ricker size= '+str(siz))

            logfile = open(filename+'.log', 'a')
            logfile.write('\nChirp Ricker Convolution'+sigfilt)
=======
            txt_edit.insert(END, '\nRicker convolution saved - Wavelet Ricker size= '+str(siz))
...
>>>>>>> ad5c44c (Atualização do repositorio)
        ...
        def save_rick_conv_sgy():
            ...
            txt_edit.insert(END, '\n')
<<<<<<< HEAD
            txt_edit.insert(END, '\nChirp Ricker Convolution Saved as SEG-Y')    
=======
            txt_edit.insert(END, '\nRicker Convolution Saved as SEG-Y')    
>>>>>>> ad5c44c (Atualização do repositorio)
        ...
        conv_chirp = Toplevel(conv_deconv)
<<<<<<< HEAD
        conv_chirp.title('CHIRP-RICKER CONVOLUTION')
=======
        conv_chirp.title('RICKER CONVOLUTION')
>>>>>>> ad5c44c (Atualização do repositorio)
        # chirp.iconbitmap("logotet.ico")
...
<<<<<<< HEAD
        btn1= Button(conv_chirp, width=20, text='Chirp-Ricker Convol', command=conv_chirp_ricker)
=======
        btn1= Button(conv_chirp, width=20, text='Ricker Convol', command=conv_chirp_ricker)
>>>>>>> ad5c44c (Atualização do repositorio)
        btn1.place(x=70, y= 110)
...
<<<<<<< HEAD
        ...
        btn4 = Button(conv_chirp, width=2, height=1, text ='?', command= lambda:pdfReader(r'C:\tethys\help\chirp.pdf'), font=('Arial', 9, 'bold'))
        btn4.place(x=10, y=240)
=======
        # btn3 = Button(conv_chirp, width=20, text=' Save SEG-Y ', command= save_rick_conv_sgy)
        # btn3.place(x=70, y= 190)
>>>>>>> ad5c44c (Atualização do repositorio)
...
<<<<<<< HEAD
    def deconv_boom():    
        ...
        def view_section():
            ...
            vm = np.percentile(sigdata, 95)
            ax.set_title('Selecionar a janela para captura do sinal do fundo ')
=======
    def deconv_bot():    
...
<<<<<<< HEAD
        ...
        def extract_pulse():
            # com os limites da arae do fundo faz a soma dos pulsos nessa janela 
            global soma, t1, t2, s1, s2
...
<<<<<<< HEAD
            fig, ax = plt.subplots(figsize=(6,4)) # mostra a janela da imagem do fundo
            vm = np.percentile(bot_win, 95)
            plt.title('Janela do fundo')
=======
            fig, ax = plt.subplots(figsize=(6,4))    # bottom window display
            vm = np.percentile(bot_win, 95)
            plt.title('Window of the bottom reflection area')
...
<<<<<<< HEAD
            def extractSmax(trace, sta, lta):   # extrai Smax (NUMERO DA AMOSTRA) do traço 
=======
            def extractSmax(trace, sta, lta):   # extracts Smax (sample nuymber) from the trace
>>>>>>> ad5c44c (Atualização do repositorio)
            ...
            n_sta = 10
<<<<<<< HEAD
            n_lta = 20 # usando os que melhor funcionaram (colocar input pra escolher)
=======
            n_lta = 20  # using the ones that worked best
>>>>>>> ad5c44c (Atualização do repositorio)
            ...
            for tr in range(t2-t1):  
<<<<<<< HEAD
                # sigtrace = bot_win[tr, s1:s2]
                smax = extractSmax(sigtrace[tr], n_sta, n_lta) 
                Smax = np.append(Smax, smax)  # numero da amostra com intensidade max de cada traço
            ...
            for i in range(t2-t1):          # interpola o intervalo do outlier
                if Smax[i] > highpass:
                    Smax[i] = Smax[i-1] # usa o anterior
            ...
            win = 5    # aplicando a media movel numa janela de win amostras
            sma = moving_average(Smax, win)
...
<<<<<<< HEAD
            ...
            # desloca os traços de acordo com a diferenca (opção usando função shift)
=======
            # difference between the spline and theshold at each point
            # corresponds to the number of samples between the two
            dif = (cs_y - Smax) 
...
<<<<<<< HEAD
            # os pulsos desta janela serao somados para produzir um pulso medio unico
=======
            # the pulses in this window will be summed to produce a single mean pulse
>>>>>>> ad5c44c (Atualização do repositorio)
            plt.figure(figsize=(3, 8))
            ...
            for t in range(t1, t2-1):
<<<<<<< HEAD
                tr1 = sigdata[t, s1:s2] # primeiro traco
                tr2 = sigdata[t+1, s1:s2] # segundo traco
=======
                tr1 = sigdata[t, s1:s2] # first trace
                tr2 = sigdata[t+1, s1:s2] # second trace
>>>>>>> ad5c44c (Atualização do repositorio)
            ...
            plt.axvline(x=0.0, color='k', linestyle='-')
<<<<<<< HEAD
            plt.title('Soma dos Traços')
            plt.savefig('Soma_dos_traços')
=======
            plt.title('Sum of Traces')
            # plt.savefig('sum_of_traces')
>>>>>>> ad5c44c (Atualização do repositorio)
            ...
            plt.plot(ap)
<<<<<<< HEAD
            plt.title('Wavetet da Autocorr')
            plt.savefig('Wavelet_Autocorr_Soma')
=======
            plt.title('Wavetet of Autocorrelation')
            # plt.savefig('wavelet_autocorr_sum')
>>>>>>> ad5c44c (Atualização do repositorio)
...
<<<<<<< HEAD
        def dec_boom_acor_pulse():   
            global bDEC, cminbDEC, cmaxbDEC, cmapbDEC, tit

            # deconvolve com a autocorrelacao
=======
        def decon_bottom():   
            global bDEC, cminbDEC, cmaxbDEC, cmapbDEC, tit
...
>>>>>>> ad5c44c (Atualização do repositorio)
            ...
            palet = sigpal 
<<<<<<< HEAD
            tit = 'Deconvolution using Bottom Signal'

            cminbDEC, cmaxbDEC, cmapbDEC = img_show(filename, tit, bDEC, tri, trf, spi, spf, sr, cmin, cmax, palet, sigfilt )

        # -----------------------------------------------------------------------------------

        def save_boom_deconv():
=======
            tit = 'Deconvolution using Bottom Signal Capture'
...
>>>>>>> ad5c44c (Atualização do repositorio)
            ...
            txt_edit.insert(END, '\n')
<<<<<<< HEAD
            txt_edit.insert(END, '\nBoomer Deconvolution saved')
            txt_edit.insert(END, '\nBottom Window between traces '+str(t1)+' and '+str(t2))
            txt_edit.insert(END, '\nBottom Window between samples '+str(s1)+' and '+str(s2))

            logfile = open(filename+'.log', 'a')
            logfile.write('\nBoomer Deconvolution'+sigfilt)
...
<<<<<<< HEAD
        ...
        deconv_boom.title('BOOMER DECONVOLUTION')
        # deconvolution.iconbitmap("logotet.ico")
        w = 320 # window width
        h = 340 # window height
        ...
        btn3 = Button(deconv_boom, width=20, text='Show Deconvol', command= dec_boom_acor_pulse)
        btn3.place(x=80, y= 170)

        btn4 = Button(deconv_boom, width=15, text='Save Deconvol', command= save_boom_deconv)
        btn4.place(x=100, y= 210)

        btn5 = Button(deconv_boom, width=20, text='Save SEG-Y', command= save_deconv_sgy)
        btn5.place(x=80, y= 250)

        btn6 = Button(deconv_boom, width=2, height=1, text ='?', command= lambda:pdfReader(r'C:\tethys\help\boomer.pdf'), font=('Arial', 9, 'bold'))
        btn6.place(x=10, y=310)
...
<<<<<<< HEAD
    btn1 = Button(conv_deconv, width=15, text='CHIRP', command= conv_chirp)
    btn1.place(x=40, y=40)
    btn2 = Button(conv_deconv, width=15, text='BOOMER', command= deconv_boom)
=======
    btn1 = Button(conv_deconv, width=15, text='CONVOLUTION', command= conv_chirp)
    btn1.place(x=40, y=40)
    btn2 = Button(conv_deconv, width=15, text='DECONVOLUTION', command= deconv_bot)
...
<<<<<<< HEAD
        LAP = ndimage.laplace(sigdata) # aplica o filtro
=======
        LAP = ndimage.laplace(sigdata) # apply the filter
>>>>>>> ad5c44c (Atualização do repositorio)
...
<<<<<<< HEAD
        cminLAP, cmaxLAP, cmapLAP = img_show(segyfile, tit, LAP, tri, trf, spi, spf, sr, cmin, cmax, palet, sigfilt)
=======
        cminLAP, cmaxLAP, cmapLAP = img_show(segyfile, tit, LAP, tri, trf, spi, spf, sr, cmin, cmax, palet, sigfilt, trace_index)
>>>>>>> ad5c44c (Atualização do repositorio)
...
<<<<<<< HEAD
        cminDX, cmaxDX, cmapDX = img_show(segyfile, tit, DX, tri, trf, spi, spf, sr, cmin, cmax, palet, sigfilt)
=======
        cminDX, cmaxDX, cmapDX = img_show(segyfile, tit, DX, tri, trf, spi, spf, sr, cmin, cmax, palet, sigfilt, trace_index)
>>>>>>> ad5c44c (Atualização do repositorio)
...
<<<<<<< HEAD
        cminDY, cmaxDY, cmapDY = img_show(segyfile, tit, DY, tri, trf, spi, spf, sr, cmin, cmax, palet, sigfilt)
=======
        cminDY, cmaxDY, cmapDY = img_show(segyfile, tit, DY, tri, trf, spi, spf, sr, cmin, cmax, palet, sigfilt, trace_index)
>>>>>>> ad5c44c (Atualização do repositorio)
...
<<<<<<< HEAD
        WN = signal.wiener(sigdata) # diminui ruido mas borra a imagem        
=======
        WN = signal.wiener(sigdata) # Reduces noise but blurs the image a little     
>>>>>>> ad5c44c (Atualização do repositorio)
...
<<<<<<< HEAD
        cminWN, cmaxWN, cmapWN = img_show(segyfile, tit, WN, tri, trf, spi, spf, sr, cmin, cmax, palet, sigfilt)
=======
        cminWN, cmaxWN, cmapWN = img_show(segyfile, tit, WN, tri, trf, spi, spf, sr, cmin, cmax, palet, sigfilt, trace_index)
>>>>>>> ad5c44c (Atualização do repositorio)
...
<<<<<<< HEAD
    ...
    btn13 = Button(enhance, width=2, height=1, text ='?', command= lambda:pdfReader(r'C:\tethys\help\img_filt.pdf'), font=('Arial', 9, 'bold'))
    ...
    def intpret():
        ...
        class PolygonDrawer:
            ...
            def __init__(self, digit):
                ...
                self.digit = digit
<<<<<<< HEAD
                self.digit.title("Desenhar Polígonos")
=======
                self.digit.title("Draw polygons")
>>>>>>> ad5c44c (Atualização do repositorio)
                self.image_path = ""
...
<<<<<<< HEAD
                self.print_button = Button(self.button_frame, text="   Sair   ", command=self.exit)
                self.print_button.pack(side=RIGHT)
                self.clear_button = Button(self.button_frame, text="    Limpar    ", command=self.clear_polygons)
                self.clear_button.pack(side=RIGHT)
                self.load_button = Button(self.button_frame, text="  Trocar Imagem  ", command=self.load_image)
                self.load_button.pack(side=RIGHT)
                self.load_image()
                self.finalize_button = Button(self.button_frame, text="  Finalizar Refletor  ", command=self.finalize_polygon)
=======
                self.print_button = Button(self.button_frame, text="   Exit   ", command=self.exit)
                self.print_button.pack(side=RIGHT)
                self.save_button = Button(self.button_frame, text=" Save Image ", command=self.save_image)
...
<<<<<<< HEAD
                    param = scenes.iloc[self.n]    # param = valores dos campos em scenes[x]
=======
                    param = scenes.iloc[self.n]    # param = field values in scenes[x]
>>>>>>> ad5c44c (Atualização do repositorio)
                    imgsig = param[0]
...
>>>>>>> ad5c44c (Atualização do repositorio)
                ...
                self.ax.set_aspect('auto')
<<<<<<< HEAD
                self.fig.tight_layout()  # Aplica o tight_layout na figura
                self.canvas.draw()
                self.current_polygon = []   
...
>>>>>>> ad5c44c (Atualização do repositorio)
            ...
            def draw_polygons(self):
                for i, polygon in enumerate(self.polygons):
<<<<<<< HEAD
                    color = self.colors[i % len(self.colors)]  # Seleciona a cor da lista de cores
=======
                    color = self.colors[i % len(self.colors)]  # Select color from color list
>>>>>>> ad5c44c (Atualização do repositorio)
                    x = [coord[0] for coord in polygon]
...
>>>>>>> ad5c44c (Atualização do repositorio)
            ...
            def finalize_polygon(self):
                if self.current_polygon:
                    ...
                    self.current_polygon = []
<<<<<<< HEAD
                    self.current_color_index += 1  # Incrementa o índice da cor atual
=======
                    self.current_color_index += 1  # Increments the current color index
>>>>>>> ad5c44c (Atualização do repositorio)
                    if self.current_color_index > 4:
        ...
        digit.attributes('-toolwindow', True) 
<<<<<<< HEAD
        digit.geometry("1600x800")  # Redefine o tamanho da janela   
=======
        digit.geometry("1600x800")     
>>>>>>> ad5c44c (Atualização do repositorio)
...
<<<<<<< HEAD
            tr = []  # array dos tracos
            sp = []  # array das amostras
            rf = []  # array dos refletores
=======
            tr = []  # array of traces
            sp = []  # array of samples
            rf = []  # array of reflectors
...
<<<<<<< HEAD
            for item in hrz: # preenche os vetores tr, sp e rf de cada linha digitalizada
=======
            for item in hrz: # fills the tr, sp and rf vectors of each scanned line
>>>>>>> ad5c44c (Atualização do repositorio)
                if hrz.index(item) % 2 == 0:
...
>>>>>>> ad5c44c (Atualização do repositorio)
            ...
            vlc = int(veloc.get())
<<<<<<< HEAD
            sound_veloc = vlc/2/1000 # velocidade do som escolhida

            # ajusta as coordenadas e a profundidade
=======
            sound_veloc = vlc/2/1000 # sound velocity
...
<<<<<<< HEAD
            size = horizonts.shape[0] # numero de vertices digitalizados
=======
            size = horizonts.shape[0] # number of scanned vertices
>>>>>>> ad5c44c (Atualização do repositorio)
...
<<<<<<< HEAD
            r1 = R[i]  # dados da primeira linha
=======
            r1 = R[i]  # first row data
>>>>>>> ad5c44c (Atualização do repositorio)
            t1 = T[i]
...
<<<<<<< HEAD
            while i < size-1:  # para todos os vertices

                if R[i] == R[i+1]:  # se for o mesmo refletor

                    t2 = T[i+1]  # pega o vertice seguinte
=======
            while i < size-1:  # for all vertices
...
<<<<<<< HEAD
                    interv = t2 - t1 + 1 # calcula interv entre tracos

                    r1 = R[i]
                    r = [r1] * interv # preenche o vetor r com o numero do reflet
                    t = [t1, t2]  # numeros dos tracos no intervalo
                    x = [x1, x2]
                    y = [y1, y2]
                    d = [d1, d2]   # profundidades no intervalo

                    t_new = np.linspace(t1, t2, interv)     # array de tr no intervalo  

                    function = interp1d(t, x)
                    x = function(t_new) # calcula x no intervalo 

                    function = interp1d(t, y)
                    y = function(t_new)  # calcula y no intervalo 

                    function = interp1d(t, d)
                    d = function(t_new)  # calcula d no intervalo 

                    t_int = t_new.astype(int)   # converte traco para inteiro                    
                    d_int = d.astype(int)   # converte prof para inteiro
                    
                    A = sigdata[t_int, d_int]  # calcula A em sig

                    # vertice seguinte agora sera o atual
=======
                    interv = t2 - t1 + 1 # calculates interval between traces
...
<<<<<<< HEAD
                    t_int = t_new.astype(int)   # converte t_new para inteiro                    
                    d_int = d.astype(int)   # converte d para inteiro
                    
                    A = sig[t_int, d_int]  # calcula A em sig
...
<<<<<<< HEAD
    btn3 = Button(interp, width=2, height=1, text ='?', command= lambda:pdfReader(r'C:\tethys\help\interpret.pdf'), font=('Arial', 9, 'bold'))
...
>>>>>>> ad5c44c (Atualização do repositorio)
                    ...
                    txt_edit.insert(END, '\n')
<<<<<<< HEAD
                    txt_edit.insert(END, 'Save DXF at: '+filename[:-4] +'_nav.dxf')
=======
                    txt_edit.insert(END, 'DXF save in: '+filename[:-4] +'_nav.dxf')
>>>>>>> ad5c44c (Atualização do repositorio)
...
>>>>>>> ad5c44c (Atualização do repositorio)
    ...
    ckb1 = Checkbutton(nav, text='Include CSV', var=chkCSV, onvalue=1, offvalue=0) 
<<<<<<< HEAD
    ckb1.place(x=70, y=20)

    btn1 = Button(nav, width=18, text='Select SEG-Y files\n for DXF export', command= exportDXF)
    btn1.place(x=40, y=60)

    btn4 = Button(nav, width=2, height=1, text ='?', command= lambda:pdfReader(r'C:\tethys\help\naveg.pdf'), font=('Arial', 9, 'bold'))
...
<<<<<<< HEAD
>>>>>>>> ad5c44c (Atualização do repositorio):tethys-1.py
=======
>>>>>>> ad5c44c (Atualização do repositorio)
root.mainloop()