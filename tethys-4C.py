''' Pendencias 
    ...
    No Select incluir as novas Deconv
    Colocar try em todas as Select para evitar problemas
    Spherical Div não precisa de Select--> RawData
    RemoveDC alterar o botão para Profile Viewer
    
    def bind_entry_with_enter(entry_widget, callback_function):
    """
    Função que faz com que um widget Entry aceite <Enter> além do TAB.
    Quando o usuário pressiona Enter dentro do Entry, chama o callback_function.
    """
    def on_enter(event):
        callback_function()
    ...
    # entry = tk.Entry(root)
    # entry.pack()
    # bind_entry_with_enter(entry, lambda: print("Valor digitado:", entry.get()))
    # root.mainloop()
...
from matplotlib.widgets import Slider, RectangleSelector, Button as Btn
...
# ===================================================================================================  
    ...
    def band_pass():
        ...
        e_LC.focus()
        e_LC.bind('<Return>', lambda event: bp_filter())   # <-- adiciona o enter
        ...
        e_HC.bind('<Return>', lambda event: bp_filter())   # <-- adiciona o enter
            ...
            def on_click(self, event):
                if event.button == 1:
                    if event.xdata is not None and event.ydata is not None:
                        ...
                        self.ax.plot(event.xdata, event.ydata, 'r', color=self.colors[self.current_color_index])
                        self.canvas.draw()
